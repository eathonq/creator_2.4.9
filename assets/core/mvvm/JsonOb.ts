...
export class JsonOb<T> {
    /**
     * 实现动态绑定的核心部分，每次修改属性值，都会调用对应函数，并且获取值的路径
     * @param obj 拦截对象
     * @param callback 回调函数
     */
    ...
    private _callback: (newVal: any, oldVal: any, pathArray: string[]) => void;

    /**对象属性劫持 */
    private observe<T>(obj: T, path?: string[]) {
     ...
     * @param path 
     */
    private overrideArrayProto(array: any, path?: string[]) {
        // 保存原始 Array 原型  
