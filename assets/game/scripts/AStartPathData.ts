...
export default class AStartPathData extends cc.Component {
    ...
    private initStartEnd() {
        ...
        node = cc.instantiate(this.templates[3]);
        node.on(cc.Node.EventType.TOUCH_START, () => {
    ...
    private initWeights() {
        let weights = [];
        let p = (x: number, y: number, w: number) => { return { x: x, y: y, weight: w } };
        weights.push(p(7, 16, 2), p(8, 16, 2));
        ...
        while (array.length > 0) {
            ...
            for (const _key in pathMap) {
                ...
                if (next.x == current.x && next.y == current.y) {
                    ...
                    array.push({ x: Number(arr[0]), y: Number(arr[1]) });

                    let node = cc.instantiate(this.templates[1]);
                    this.numberLayer.addChild(node);
                    node.setPosition(this.tiles[next.x][next.y].getPosition());
                    node.getComponent(cc.Label).string = `${level}`;
                }
            }

            if (pre == `${current.x}_${current.y}`) {
                pre = `${current.x}_${current.y}`;
                level++;
            }
        }
    }
